...
StateQMFixedStand::StateQMFixedStand(CtrlInterfaces& ctrl_interfaces, const std::vector<double>& target_pos,
                               const double kp,
                               const double kd)
    : FSMState(FSMStateName::QMFIXEDSTAND, "qm fixed stand", ctrl_interfaces),
      kp_(kp), kd_(kd)
{
...
void StateQMFixedStand::run(const rclcpp::Time&/*time*/, const rclcpp::Duration&/*period*/)
{
...
void StateQMFixedStand::exit()
{
    percent_ = 0;
}

FSMStateName StateQMFixedStand::checkChange()
{
    if (percent_ < 1.5)
    {
