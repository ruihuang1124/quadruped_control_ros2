...
#include "hardware_mujoco_quadruped_wheel/HardwareMujocoQuadrupedWheel.h"
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn HardwareMujocoQuadrupedWheel::on_init(
    const hardware_interface::HardwareInfo &info)
{
    ...
    manipulator_joint_state_subscriber_ = node_->create_subscription<sensor_msgs::msg::JointState>(
        "joint_states_single", rclcpp::SensorDataQoS(), std::bind(&HardwareMujocoQuadrupedWheel::manipulator_joint_state_callback, this, std::placeholders::_1));

    quadruped_joint_state_subscriber_ = node_->create_subscription<sensor_msgs::msg::JointState>(
    "joint_states", rclcpp::SensorDataQoS(), std::bind(&HardwareMujocoQuadrupedWheel::quadruped_joint_state_callback, this, std::placeholders::_1));

    imu_subscriber_ = node_->create_subscription<sensor_msgs::msg::Imu>(
        "imu_data", rclcpp::SensorDataQoS(), std::bind(&HardwareMujocoQuadrupedWheel::imu_callback, this, std::placeholders::_1));
...
std::vector<hardware_interface::StateInterface> HardwareMujocoQuadrupedWheel::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> HardwareMujocoQuadrupedWheel::export_command_interfaces()
{
...
return_type HardwareMujocoQuadrupedWheel::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
...
return_type HardwareMujocoQuadrupedWheel::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
    // TODO: emplace_back or push_back
    custom_msgs::msg::ActuatorCmds actuator_cmds;
    // sensor_msgs::msg::JointState joint_cmds;

    for (size_t i = 0; i < dof_quadruped_wheel; i++)
    {
...
void HardwareMujocoQuadrupedWheel::imu_callback(const sensor_msgs::msg::Imu imu_state)
{
...
void HardwareMujocoQuadrupedWheel::manipulator_joint_state_callback(const sensor_msgs::msg::JointState joint_state)
{
...
void HardwareMujocoQuadrupedWheel::quadruped_joint_state_callback(const sensor_msgs::msg::JointState joint_state)
{
    for (size_t i = 0; i < joint_state.name.size(); i++)
    {
        joint_position_states_[joint_state.name[i]] = joint_state.position[i];
...
PLUGINLIB_EXPORT_CLASS(HardwareMujocoQuadrupedWheel, hardware_interface::SystemInterface)
