...
namespace ArcLab
{
    QWMujocoMsgHandler::QWMujocoMsgHandler(mj::Simulate *sim)
        : Node("QWMujocoMsgHandler", rclcpp::NodeOptions().use_intra_process_comms(true)),
          sim_(sim)
    {
        ...
        actuator_cmd_subscription_ = this->create_subscription<custom_msgs::msg::ActuatorCmds>(
            "actuators_cmds",
            qos,
            std::bind(&QWMujocoMsgHandler::actuator_cmd_callback, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "Start QWMujocoMsgHandler ...");
        sim_->uiloadrequest.fetch_add(1);
    }

    QWMujocoMsgHandler::~QWMujocoMsgHandler()
    {
        RCLCPP_INFO(this->get_logger(), "close node ...");
    }

    void QWMujocoMsgHandler::publish_mujoco_callback()
    {
        if (sim_->d_ != nullptr)
        {
    ...
    void QWMujocoMsgHandler::imu_callback()
    {
    ...
    void QWMujocoMsgHandler::joint_callback()
    {
        if (sim_->d_ != nullptr)
        {
    ...
    void QWMujocoMsgHandler::actuator_cmd_callback(
