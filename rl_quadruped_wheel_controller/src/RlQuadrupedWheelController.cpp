...
#include "RlQuadrupedWheelController.h"

namespace rl_quadruped_wheel_controller
{
    using config_type = controller_interface::interface_configuration_type;

    controller_interface::InterfaceConfiguration LeggedWheelController::command_interface_configuration() const
    {
    ...
    controller_interface::InterfaceConfiguration LeggedWheelController::state_interface_configuration() const
    {
        controller_interface::InterfaceConfiguration conf = {config_type::INDIVIDUAL, {}};
    ...
    controller_interface::return_type LeggedWheelController::
    ...
    controller_interface::CallbackReturn LeggedWheelController::on_init()
    {
        try
        {
    ...
    controller_interface::CallbackReturn LeggedWheelController::on_configure(
    ...
    controller_interface::CallbackReturn LeggedWheelController::on_activate(
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        ...
        // Create FSM List
        state_list_.passive = std::make_shared<StateQWPassive>(ctrl_interfaces_);
        state_list_.fixedDown = std::make_shared<StateQWFixedDown>(ctrl_interfaces_, down_pos_, stand_kp_, stand_kd_);
        state_list_.fixedStand = std::make_shared<StateQWFixedStand>(ctrl_interfaces_, stand_pos_, stand_kp_, stand_kd_);
        state_list_.rl = std::make_shared<StateQWRL>(ctrl_interfaces_, ctrl_component_, stand_pos_);
    ...
    controller_interface::CallbackReturn LeggedWheelController::on_deactivate(
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        release_interfaces();
        return CallbackReturn::SUCCESS;
    ...
    LeggedWheelController::on_cleanup(const rclcpp_lifecycle::State& previous_state)
    {
        return ControllerInterface::on_cleanup(previous_state);
    }

    controller_interface::CallbackReturn
    LeggedWheelController::on_shutdown(const rclcpp_lifecycle::State& previous_state)
    {
        return ControllerInterface::on_shutdown(previous_state);
    }

    controller_interface::CallbackReturn LeggedWheelController::on_error(const rclcpp_lifecycle::State& previous_state)
    {
        return ControllerInterface::on_error(previous_state);
    }

    std::shared_ptr<FSMState> LeggedWheelController::getNextState(const FSMStateName stateName) const
    {
        switch (stateName)
        {
        case FSMStateName::INVALID:
            return state_list_.invalid;
        case FSMStateName::QWPASSIVE:
            return state_list_.passive;
        case FSMStateName::QWFIXEDDOWN:
            return state_list_.fixedDown;
        case FSMStateName::QWFIXEDSTAND:
            return state_list_.fixedStand;
        case FSMStateName::QWRL:
...
PLUGINLIB_EXPORT_CLASS(rl_quadruped_wheel_controller::LeggedWheelController, controller_interface::ControllerInterface);
