...
namespace ocs2::legged_robot
{
    ...
    {
        const size_t size = ctrl_component_.joint_position_state_interface_.size();
        
        if (size < info_.actuatedDofNum) {
            RCLCPP_ERROR_ONCE(node_->get_logger(), "State estimator: joint state interface size (%zu) < actuatedDofNum (%zu)", size, info_.actuatedDofNum);
            return;
        }

        vector_t joint_pos(info_.actuatedDofNum), joint_vel(info_.actuatedDofNum);
        ...
        std::cerr << "[DEBUG] updateJointStates: rbd_state_.size()=" << rbd_state_.size() << ", info_.generalizedCoordinatesNum=" << info_.generalizedCoordinatesNum << ", info_.actuatedDofNum=" << info_.actuatedDofNum << "\n";
