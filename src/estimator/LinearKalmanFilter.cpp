...
namespace ocs2::legged_robot
{
    ...
    vector_t KalmanFilterEstimate::update(const contact_flag_t &contactFlag, const rclcpp::Time &time, const rclcpp::Duration &period)
    {
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: entering");
        updateJointStates();
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: updateJointStates done");
        updateContact(contactFlag);
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: updateContact done");
        updateImu();
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: updateImu done");
        ...
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: solving s.lu().solve(ey)...");
        vector_t sEy = s.lu().solve(ey);
        xHat_ += pm * cT * sEy;

        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: solving s.lu().solve(c_)...");
        ...
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: updateLinear...");
        updateLinear(xHat_.segment<3>(0), xHat_.segment<3>(3));

        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: getOdomMsg...");
        auto odom = getOdomMsg();
        odom.header.stamp = time;
        odom.header.frame_id = "odom";
        odom.child_frame_id = "base";
        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: publishMsgs...");
        publishMsgs(odom);

        LOG_TRACE("[DEBUG] KalmanFilterEstimate::update: returning rbd_state_");
        return rbd_state_;
    }
