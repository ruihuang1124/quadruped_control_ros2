...
namespace ocs2::legged_robot
{
    TargetManager::TargetManager(CtrlComponent &ctrl_component,
        ...
        : ctrl_component_(ctrl_component),
          ...
          node_(std::move(node))
    {
        default_joint_state_ = vector_t::Zero(16);
    ...
    {
        double cmd_linear_x, cmd_linear_y, cmd_angular_z, cmd_height_ratio;
        {
            std::lock_guard<std::mutex> lock(ctrl_component_.user_cmds_mutex_);
            cmd_linear_x = ctrl_component_.user_cmds_.linear_x_input;
            cmd_linear_y = ctrl_component_.user_cmds_.linear_y_input;
            cmd_angular_z = ctrl_component_.user_cmds_.angular_z_input;
            cmd_height_ratio = ctrl_component_.user_cmds_.height_ratio;
        }

        vector_t cmdGoal = vector_t::Zero(6);
        // cmdGoal[vx, vy, vz, wz] is expressed in body frame
        cmdGoal[0] = cmd_linear_x * target_displacement_velocity_;
        cmdGoal[1] = cmd_linear_y * target_displacement_velocity_;
        cmdGoal[2] = 0;
        cmdGoal[3] = cmd_angular_z * target_rotation_velocity_;
        ...
        targetPose(2) = cmd_height_ratio * command_height_; // z
        ...
        trajectories.stateTrajectory[0](3) = cmdGoal(3);
        trajectories.stateTrajectory[1](3) = cmdGoal(3);
